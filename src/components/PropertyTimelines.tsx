...
interface PropertyTimelinesProps {
  userId: string;
}

const PropertyTimelines: FunctionComponent<PropertyTimelinesProps> = ({
  userId,
}) => {
  const fetchPropertyTimelines = async () => {
    const response = await fetch(
      `/api/properties/timelines/?userId=${userId}&page=0`,
      {
        method: "GET",
      }
    );
  ...
  return (
    <div className="mt-4 bg-white">
      {data && data.length > 0 ? (
        ...
        <p className="text-center text-gray-500 mt-4">There&apos;s no posts</p>
      )}
    </div>
  );
};

export default PropertyTimelines;
